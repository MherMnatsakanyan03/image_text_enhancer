...
\title[Parallelized Document Enhancement in WebAssembly]{Parallelized Document Enhancement in WebAssembly}
...
With increasing heterogeneity of compute environments, portable execution environments have grown in importance. This work demonstrates that a fully featured document-enhancement pipeline implemented with mature general-purpose C++ libraries, CImg and OpenMP, can be compiled for WebAssembly (WASM) with low effort. As a proof of concept, we compile the pipeline and its dependencies to WASM and run it in a Chromium-based browser using the V8 engine. The results confirm that the application executes correctly in this platform-independent setting and achieves comparable runtime performance to a native x86 build for limited workloads. Overall, our findings support WASM as a possible target for porting existing C++ applications and therefore a promising building block for portable execution across heterogeneous systems.

%% This paper examines the feasibility of running a parallel, SIMD-ready, fully featured document-enhancement pipeline using general C++ libraries in a platform-independent setting--an increasingly relevant goal as compute environments become more heterogeneous and WebAssembly (WASM) matures as an execution target \cite{exploring_web_assembly_in_hpc_chadha_2023}. We ask whether the pipeline and its dependencies can be compiled to WASM and executed in practice without major compromises in runtime performance compared to a native x86 build. As a proof of concept, we ran the performance tests on the pipeline using the V8 engine of a Chromium based browser. The experiment demonstrates that the pipeline can be executed successfully in this setting and that its runtime performance is broadly comparable to the x86 version for limited workloads. These results indicate that, WASM is a promising target for existing parallel algorithms, pointing toward the broader potential of portable execution models for heterogeneous systems.


%% with focus in HPC
%% This paper investigates whether a parallel, fully featured document-enhancement pipeline can run in a platform-independent environment -- an increasingly relevant goal as compute clusters become more heterogeneous and WebAssembly (WASM) gains interest in high-performance computing \cite{exploring_web_assembly_in_hpc_chadha_2023}. We ask whether the pipeline and its full set of dependencies can be compiled to WASM and executed without sacrificing the capabilities expected from a native x86 implementation. To answer this, we ported the pipeline and dependencies to WASM and evaluated the resulting system's runtime performance against the x86 baseline. Our results show that the WASM implementation achieves runtime performance comparable to the x86 version, without major compromises. These findings suggest that WASM could become a deployment target for parallel workloads and supports the broader case for using WASM to improve portability.
...
\keywords{document enhancement, parallel, OpenMP, CImg, pipeline, platform independent, WebAssembly}
...
\begin{enumerate}
  ...
  \item all of the above, and it can be compiled to run as WASM (compare to \href{https://silvia-odwyer.github.io/photon/}{Photon} and \href{https://www.npmjs.com/package/wasm-vips}{wasm-vips})! The question would be: How easy is it to compile a C++ image processing pipeline with OpenMP and CImg to WASM? (This would show, that you can take a lightweight C++ library and just put it in the browser with great parallelisation.)
  \item We could additionally try and implement one of the filters or binarization methods in WASM SIMD (which is not platform specific) and show which of the two is faster. The conclusion could then be: is it worth it to manually write SIMD for WASM or is OpenMP good enough (which would be an interesting conclusion)?
\end{enumerate}
...
\begin{comment}
  WASM single threaded: 2407.4 ms
  WASM 32 vCores OpenMP: 666.29 ms
  Local single threaded: 1437.51 ms
  Local 32 vCores OpenMP: 527.78 ms
