...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Image loaded with dimensions: " << img.width() << "x"
            << img.height() << "x" << img.depth() << "x" << img.spectrum()
            << std::endl;

  // Create the directory if it doesn't exist
  if (!std::filesystem::is_directory(output_dir) &&
      !std::filesystem::is_regular_file(output_dir)) {
    std::filesystem::create_directory(output_dir);
    std::cout << "Created directory: " << output_dir.string() << std::endl;
  }

  auto enhanced_img =
      ite::enhance(img, 0.0f, 1, 3, true, false, false, true, true, true);
