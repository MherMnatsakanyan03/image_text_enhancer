...
int main(int argc, char* argv[]) {
    ...
    std::filesystem::path executable_path =
        (argc > 0) ? std::filesystem::canonical(argv[0])
                   : (std::filesystem::current_path() / "ITE");
    ...
    for (const auto& entry : std::filesystem::directory_iterator(input_dir)) {
        ...
        if (!is_image_file(in_path)) continue;

        try {
            auto img = ite::loadimage(in_path.string());

            std::cout << "Loaded: " << in_path.filename().string()
                      << " (" << img.width() << "x" << img.height()
                      << "x" << img.depth() << "x" << img.spectrum() << ")\n";

            auto output_img = ite::enhance(img, 1.0f, 1, 5, true, true, false, true, true);
            ...
            std::cerr << "Failed processing " << in_path.string()
                      << " : " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "Failed processing " << in_path.string()
    ...
    std::cout << "Done. Processed " << processed
              << " image(s). Output dir: " << output_dir.string() << std::endl;
