...
#pragma omp simd reduction(+ : acc)
                    ...
                    // Write back
                    for (int x = 0; x < w; ++x)
                        row[x] = clamp_u8_from_float(tmp[x]);
                }
            }
        }
    }
...
#pragma omp simd reduction(+ : acc)
                    ...
                    // Write back
                    for (int y = 0; y < h; ++y)
                        base[(size_t)y * w + x] = clamp_u8_from_float(tmp[y]);
                }
            }
        }
    }
                                               ...
                                               int truncate = 3, int block_h = 64)
{
    ...
    if (w <= 1 || h <= 1)
    {
        ...
        if (sigma_low > 0.0f)
            gaussian_blur_inplace_omp(img, sigma_low, truncate);
        return;
    }
...
namespace ite
{
                       ...
                       bool do_dilation, bool do_despeckle, bool do_deskew)
    {
        ...
        // Binarize
        binarize_inplace(l_image, 15, 0.2f, 4.0f);
