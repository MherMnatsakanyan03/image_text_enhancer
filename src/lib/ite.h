...
/**
 * @namespace ite
 * @brief Contains functions for Image Text Enhancement (ITE) to improve OCR
 * readability.
 */
namespace ite {
...
/**
 * @brief Converts a grayscale image inplace to a binary (black and white)
 * image. If the image is not grayscale, it is first converted to grayscale.
 ...
 * This operation thickens bright regions. On a binary image, it connects broken
 * character parts.
 * @param input_image The source image (typically binary).
 * @param kernel_size The size of the structuring element (e.g., 3 for a 3x3
 * kernel).
 ...
 * This operation thickens dark regions. On a binary image, it removes small
 * noise specks.
 * @param input_image The source image (typically binary).
 * @param kernel_size The size of the structuring element (e.g., 3 for a 3x3
 * kernel).
 ...
 * @param threshold Components smaller than this number of pixels will be
 * removed.
 * @param diagonal_connections Whether to consider diagonal connections as
 * connected.
 ...
 * @param kernel_size The structuring element size for morphological operations
 * (default 3).
 * @param despeckle_threshold The size threshold for despeckling (default 5).
 * @param diagonal_connections Whether to consider diagonal connections in
 * despeckling (default true).
 ...
 * @param do_adaptive_binarization Whether to use Bataine's adaptive
 * binarization (default false).
 * @return A new, enhanced image ready for OCR.
 */
CImg<uint> enhance(const CImg<uint> &input_image, float sigma = 1.0f,
                   int kernel_size = 3, int despeckle_threshold = 5,
                   bool diagonal_connections = true, bool do_erosion = false,
                   bool do_dilation = true, bool do_despeckle = true,
                   bool do_deskew = true,
